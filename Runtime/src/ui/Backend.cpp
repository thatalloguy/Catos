//
// Created by allos on 28/05/2025.
//
...
struct BackendData {
	...
	catos::Window* window = nullptr;

	bool running = true;
};
static Rml::UniquePtr<BackendData> data;

bool Backend::Initialize(catos::Window& window)
{
	RMLUI_ASSERT(!data);
	...
	data->system_interface.SetWindow((SDL_Window*) window.get_raw_window_ptr());
	data->render_interface.SetViewport(window.getSize().getX(), window.getSize().getY());
...
//  The window is destroyed but the engine itself not us.
//	SDL_DestroyWindow((SDL_Window*) data->window->get_raw_window_ptr());
...
bool Backend::ProcessEvents(Rml::Context* context, KeyDownCallback key_down_callback, bool power_save)
{
	...
	auto GetDisplayScale = []() { return SDL_GetWindowDisplayScale((SDL_Window*) data->window->get_raw_window_ptr()); };
	...
	SDL_GL_SwapWindow((SDL_Window*) data->window->get_raw_window_ptr());
	...
	while (has_event)
	{
		...
		case event_key_down:
		{
			...
			if (!RmlSDL::InputEventHandler(context, (SDL_Window*) data->window->get_raw_window_ptr(), ev))
				break;
		...
		if (propagate_event)
			RmlSDL::InputEventHandler(context, (SDL_Window*) data->window->get_raw_window_ptr(), ev);
