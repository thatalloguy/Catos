...
namespace catos {
    ...
    struct Query {
        ...
        Query(World& world) : p_world(&world) {
            ...
            if (sizeof...(Components) == 0) {
                ...
                for (int i = 1; i < (sizeof...(Components) + 1); i++) {
                    componentMask.set(component_ids[i], true);
                }
            }
        ...
        const Iterator begin() const {
            int firstIndex = 0;
            while (firstIndex < p_world->entities.size() &&
                   (componentMask != (componentMask & p_world->entities[firstIndex].mask)
                    || !EntityUtils::is_entity_valid(p_world->entities[firstIndex].id)))
            {
                firstIndex++;
            }

            return Iterator(p_world, firstIndex, componentMask, all);
        };
